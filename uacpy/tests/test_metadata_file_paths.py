...
Every model attaches its primary output paths (and ``prt_file``) to
``result.metadata`` when ``cleanup=False`` (i.e. files persist past
``run()``). When ``cleanup=True`` the work dir is wiped and the keys
are absent — that's the discoverable signal.
...
def test_bounce_paths_absent_when_no_work_dir():
    ...
    # File paths absent — work dir was wiped
    assert 'brc_file' not in res.metadata
    assert 'irc_file' not in res.metadata
...
def test_sparc_paths_absent_when_cleanup_true():
    ...
    field = sp.run(env, src, rcv)
    assert 'rts_file' not in field.metadata
    assert 'grn_file' not in field.metadata
    assert 'prt_file' not in field.metadata
...
def test_bellhop_pinned_work_dir_with_cleanup_true_is_wiped(tmp_path):
    ...
    after run(); the absence of file-path keys mirrors the on-disk state."""
    env, src, rcv = _basic_setup()
