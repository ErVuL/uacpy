...
class TestThorpReferenceValues:
    ...
    def test_matches_published_curve(self, f_hz, a_dB_km):
        ...
        assert float(_thorp_dB_per_km(f_hz)) == pytest.approx(
            a_dB_km, rel=1e-12)
    ...
    def test_class_converts_dB_per_km_to_dB_per_m(self):
        ...
        depth."""
        from uacpy.core.absorption import Thorp, _thorp_dB_per_km
        z = np.array([0.0, 500.0, 5000.0])
        a = Thorp().alpha_dB_per_m(1e4, z)
        assert a.shape == z.shape
        np.testing.assert_allclose(
            a, float(_thorp_dB_per_km(1e4)) / 1000.0, rtol=1e-12)
...
class TestFrancoisGarrisonReferenceValues:
    ...
    def test_matches_published_curve(self, f_hz, T, S, pH, z, a_dB_km):
        ...
        got = float(_francois_garrison_dB_per_km(f_hz, T, S, pH, z))
    ...
    def test_the_a3_branch_at_exactly_20_degrees_is_the_warm_fit(self):
        ...
        assert float(_francois_garrison_dB_per_km(f_khz * 1e3, T, 35.0, 8.0, z)) \
    ...
    def test_depth_correction_attenuates_mgso4_term(self):
        ...
        from uacpy.core.absorption import _francois_garrison_dB_per_km
        a_surf = float(_francois_garrison_dB_per_km(6.3e4, 10., 35., 8., 0.))
        a_deep = float(_francois_garrison_dB_per_km(6.3e4, 10., 35., 8., 4000.))
    ...
    def test_class_overrides_nominal_depth_with_the_depth_axis(self):
        ...
        from uacpy.core.absorption import (
            FrancoisGarrison, _francois_garrison_dB_per_km)
        ...
        want = _francois_garrison_dB_per_km(
...
# ─────────────────────────────────────────────────────────────────────────────
# acoustic_signal/estimate.py — frequency ladder, kernel, frames, hop, ppsd
# ─────────────────────────────────────────────────────────────────────────────
...
class TestConstantQKernelConstruction:
    ...
    def test_kernel_matches_definition_exactly(self):
        ...
        from uacpy.acoustic_signal.estimate import _cq_kernels
...
class TestConstantQFrameGeometry:
    ...
    def test_window_is_centred_on_the_requested_sample(self):
        from uacpy.acoustic_signal.estimate import _cq_frame, _cq_kernels
...
class TestConstantQHopResolution:
    ...
    *lowest* bin's window; an explicit ``hop=1`` is legal."""
    ...
    def test_default_hop_follows_the_lowest_bin(self):
        from uacpy.acoustic_signal.estimate import _resolve_hop
...
class TestProbabilisticConstantQContracts:
    ...
    def test_level_edges_run_from_lvlmin_to_lvlmax_inclusive(self):
        ...
        r = probabilistic_constant_q(x, fs, scaling='spectrum', fmin=fmin, bins_per_octave=B,
    ...
    def test_single_valid_frame_is_data(self):
        ...
        x, fs, fmin, B, n_lowest = self._one_frame_case()
        psd = constant_q(x, fs, scaling='spectrum', fmin=fmin,
                             bins_per_octave=B, hop=n_lowest)
        assert np.isfinite(psd.power[0])
        ppsd = probabilistic_constant_q(x, fs, scaling='spectrum', fmin=fmin, bins_per_octave=B,
                                            hop=n_lowest)
...
# ─────────────────────────────────────────────────────────────────────────────
# acoustic_signal/arrays.py — radon/taup/fk contract holes (wave-5)
# ─────────────────────────────────────────────────────────────────────────────
...
class TestParabolicMoveoutCurve:
    ...
    def test_matches_the_closed_form(self):
        from uacpy.acoustic_signal.arrays import _moveout_times
...
class TestTaupDefaultSlownessCeiling:
    ...
    def test_default_p_max(self):
        from uacpy.acoustic_signal.arrays import taup_transform
...
# ─────────────────────────────────────────────────────────────────────────────
# acoustic_signal/estimate.py — lifter, smoothing windows, cwt, cepstra
# ─────────────────────────────────────────────────────────────────────────────
...
class TestCepstralLifterMask:
    ...
    def test_low_pass_mask_exact(self):
        from uacpy.acoustic_signal.estimate import _apply_lifter
        c = np.arange(1.0, 9.0)
...
class TestWignerSmoothingWindowConstruction:
    ...
    def test_none_is_no_smoothing(self):
        from uacpy.acoustic_signal.estimate import _smoothing_window
...
class TestWignerVilleTimeAxis:
    ...
    def test_times_axis(self):
        from uacpy.acoustic_signal.estimate import wigner_ville
        fs = 500.0
...
class TestCwtFrequencyContract:
    ...
    def test_sub_hertz_sample_rate_is_legal(self):
        from uacpy.acoustic_signal.estimate import cwt
...
class TestCepstrumWindowConvention:
    ...
    def test_matches_prewindowed_signal(self):
        import scipy.signal as _sig
        from uacpy.acoustic_signal.estimate import cepstrum
...
class TestComplexCepstrumDelayEstimator:
    ...
    def test_delayed_impulse_reports_the_ramp(self):
        from uacpy.acoustic_signal.estimate import complex_cepstrum
        x = np.zeros(31)
        x[3] = 1.0
...
class TestProbabilisticConstantQDefaultLevels:
    ...
    def test_default_edges(self):
        ...
        r = probabilistic_constant_q(x, 1000.0, scaling='spectrum', fmin=100.0,
                                     bins_per_octave=2)
        assert r.level_edges[0] == 0.0
        assert r.level_edges[-1] == 150.0
...
class TestInverseRadonOffsetAxisOrigin:
    ...
    def test_zero_offset_trace_takes_tau(self):
        from uacpy.acoustic_signal.arrays import inverse_radon
