"""The four OASES variants, each asked for what only it computes.

OASES (Ocean Acoustics and Seismic Exploration System) is a suite of
seismo-acoustic wavenumber-integration models, and uacpy wraps one variant
per question:

- OAST: transmission loss
- OASN: array covariance and signal replicas under surface/discrete noise
- OASR: reflection coefficients
- OASP: pulse / broadband transfer functions

Each is driven end to end here — deck, run, parse — so a variant that stops
producing its own result is caught at the variant that owns it rather than in
