...
class TestSspRecordsSpanLines:
    ...
    def test_3d_rows_may_wrap(self, tmp_path):
        from uacpy.io.oalib_reader import read_ssp_3d
        p = tmp_path / 'w3.ssp'
        p.write_text("2\n0.0\n10.0\n2\n0.0 10.0\n2\n0.0\n100.0\n"
                     "1500 1501\n1502\n1503\n1504 1505\n1506 1507\n")
        d = read_ssp_3d(p)
        assert d['c_mat'].ravel().tolist() == [
...
class TestOastTlMultiFrequency:
    ...
    def test_single_frequency_keeps_2d_shape(self, tmp_path):
        ...
        tl, depths, ranges, meta = read_oast_tl(plp, [10.0, 20.0])
        assert tl.shape == (2, 4)
        assert meta['n_frequencies'] == 1
        assert ranges.tolist() == [1000.0, 1500.0, 2000.0, 2500.0]

    def test_multi_frequency_returns_a_stack(self, tmp_path):
        from uacpy.io.oases_reader import read_oast_tl
        plp = self._write_pair(tmp_path, n_freq=3, n_depths=2)
        tl, _d, _r, meta = read_oast_tl(plp, [10.0, 20.0])
        assert tl.shape == (3, 2, 4)
        assert meta['n_frequencies'] == 3
        # Frequency-major order: curve (ifreq, idepth) = ifreq*2 + idepth.
        assert tl[1, 1, 0] == 300.0
        assert tl[2, 0, 0] == 400.0
...
class TestBoundaryFileGuards:
    ...
    def test_nan_depth_grid_is_rejected(self, tmp_path):
        from uacpy.io.bathy_io import write_bty_3d
        with pytest.raises(ConfigurationError, match='non-finite'):
            write_bty_3d(tmp_path / 'n.bty', np.array([0.0, 100.0]),
                         np.array([0.0, 100.0]),
...
class TestShdMultiSourcePositionRefused:
    """``read_shd_file`` returns fields for one source (x, y) position; a
    BELLHOP3D/FIELD3D file with Nsx/Nsy > 1 is refused by name rather than
    silently reduced to slot (0, 0)."""
    ...
    def test_nsx_greater_than_one_raises(self, tmp_path, monkeypatch):
        ...
        def fake_bin(path):
            return {
                'freqVec': np.array([100.0]),
                'PlotType': 'rectilin  ',
                'Pos': {'theta': np.array([0.0]),
                        's': {'x': np.array([0.0, 1000.0]),
                              'y': np.array([0.0]),
                              'z': np.array([5.0])},
                        'r': {'z': np.array([10.0, 20.0]),
                              'r': np.array([100.0, 200.0])}},
                'pressure': np.ones((1, 1, 2, 2), dtype=complex),
        ...
        with pytest.raises(UnsupportedFeatureError, match='Nsx=2'):
            oalib_reader.read_shd_file(tmp_path / 'x.shd')
