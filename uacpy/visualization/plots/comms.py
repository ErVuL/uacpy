...
def plot_channel(h, sample_rate, axes=None, *, title=None, figsize=(12, 4)):
    """Two-panel channel view: |h[n]| (delay) and |H(f)| (frequency response).
    ``axes`` may be a 2-tuple ``(ax_delay, ax_freq)``."""
    ...
    axes[0].set_ylabel("|h|")
    axes[0].set_title(title or "Channel impulse response", loc="left")
    ...
    axes[1].set_title("Frequency response", loc="left")
