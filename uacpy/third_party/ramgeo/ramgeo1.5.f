...
c
c     UACPY: enlarged from (mr=100,mz=8000,mp=10) so uacpy's fine Lytaev
c     range/depth grids fit (matches the patched ramsurf1.5.f dimensions).
...
c     UACPY: complex envelope u*f3/sqrt(r) per output range, mirroring
c     tl.grid's record geometry, so uacpy can assemble a broadband H(f)
c     by looping frequencies (same scheme as the patched ramsurf1.5.f).
...
c     UACPY: same envelope as tlg, with the cylindrical-spreading factor;
c     carrier exp(+i k0 r) is factored out — the Python wrapper bakes the
c     engineering travelling-wave carrier exp(-i k0 r) in before tagging.
      urg(j)=ur/sqrt(r+eps)
    1 continue
