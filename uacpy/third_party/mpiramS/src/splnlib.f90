...
! From:  http://people.scs.fsu.edu/~burkardt/f_src/pppack/pppack.html
! BDD 5/08
...
!
  ...
  real ( kind = wp2 ) c(4,n)
  real ( kind = wp2 ) divdf1
  real ( kind = wp2 ) divdf3
  real ( kind = wp2 ) dtau
  real ( kind = wp2 ) g
  integer i
  integer ibcbeg
...
!
!    Input, integer K, the order of the polynomial pieces that make up 
!    the function F.  The usual value for K is 4, signifying a piecewise 
!    cubic polynomial.
!
