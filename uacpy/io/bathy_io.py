...
def write_bty_long_format(
    ...
    holds from the midpoint with its left neighbour to the midpoint with its
    right (the nearest-column rule of ``Bottom.at`` / ``Bottom.halfspace_at``,
    the one Kraken's segments and RAM's profiles switch on too). Bellhop
    applies a row's geoacoustics to the whole segment to its right
    (``bellhop.f90:478``, ``Bot(IsegBot)%HS``), so the rows are the union of
    the bathymetry nodes, the first bottom node and the MIDPOINTS between
    consecutive bottom nodes, and every row carries the column that owns the
    segment starting at it. That reproduces the step exactly with no extra
    rows. Depth is linearly interpolated onto every row — bathymetry IS a
    ramp between its nodes, which is Bellhop's own ``'L'`` semantics.
    """
    filepath = Path(filepath)
    ...
    switches_km = 0.5 * (rd_r_km[:-1] + rd_r_km[1:])
    r_km = np.union1d(bathy_km[:, 0], rd_r_km[:1])
    # Drop a switch within 1 mm of an existing row: the merged-axis check
    # below refuses tokens that print identically at six decimals, and the
    # row already there takes the switch's column through the segment
    # centre lookup.
    new_switches = switches_km[
        np.abs(switches_km[:, None] - r_km[None, :]).min(axis=1) > 1e-6]
    r_km = np.union1d(r_km, new_switches)
    ...
    # The column a row carries is the one owning the segment to its right:
    # evaluated at the segment's centre (the last row at itself), so a row
    # sitting exactly on a switch takes the column to the right of it, and
