...
def main():
    ...
    if tf_models:
        ...
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / 'example_28_transfer_functions.png',
                    dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"\nSaved: {OUTPUT_DIR / 'example_28_transfer_functions.png'}")
    ...
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / 'example_28_tl_depth_comparison.png',
                dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved: {OUTPUT_DIR / 'example_28_tl_depth_comparison.png'}")
    ...
    if ts_results:
        ...
        fig.suptitle(f'Time-Series Comparison — depth={target_depth:.0f} m, '
                     ...
                     fontsize=13)
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / 'example_28_time_series_comparison.png',
                    dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved: {OUTPUT_DIR / 'example_28_time_series_comparison.png'}")
    ...
    if 'Bellhop (chirp)' in results:
        ...
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / 'example_28_bellhop_chirp.png',
                    dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved: {OUTPUT_DIR / 'example_28_bellhop_chirp.png'}")
    ...
    if 'SPARC' in results:
        ...
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / 'example_28_sparc_time_series.png',
                    dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved: {OUTPUT_DIR / 'example_28_sparc_time_series.png'}")
