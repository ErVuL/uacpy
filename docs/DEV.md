...
- Reach for `CLAUDE.md` first — it has the densest architectural map.
- `AUDIT.md` and `AUDIT_SCIENCE.md` log known correctness risks by
  finding ID (SC-…, SH-…, SM-…, SL-…); cross-reference if you suspect
  an existing bug.
- `DOCUMENTATION.md` is the canonical user-facing reference. Update it
  in the same PR as any user-visible signature change.
- `third_party/MODIFICATIONS.md` is the canonical log of vendored-source
  changes. Update it in the same PR as any vendored edit.
