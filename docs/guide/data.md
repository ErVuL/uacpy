...
stamps every layer with where it came from. Every figure on this page is
generated by [`docs/figure_scripts/data.py`](../figure_scripts/data.py); the
snippets below are condensed from it, so the script is the authoritative
figure code.
...
- **`diesing`** covers deep sea only (> 500 m), so its shelf-end values are the
  first covered point held backwards. Transect fetchers fill each gap from the
  **nearest covered** waypoint (by along-track distance, whichever side of the
  gap it lies on) and raise only if *no* point on the track is covered — a flat
  section can mean uniform seabed or no coverage, and only the source's
  documented footprint tells you which.
...
| `nbs` | `altimetry` | global | 0.25°, monthly climatology (cached) or 6-hourly sampled at 00:00 UTC (live) | public domain (NOAA) | ✅ `--data wind` |
...
- **`DataSource`** is a *catalogue entry*: one immutable record per dataset,
  ...
  `commercial_use` flag. `SOURCES` is the single source of truth — 21 entries,
...
data.cache_root()             # the directory in use
...
inside a dataset. An unknown dataset name raises — that is a typo, not an
uninstalled dataset.

A missing dataset raises a typed `ConfigurationError` naming the exact flag:

```
Offline gebco data not found: GEBCO 2025 global bathymetry grid (~7.5 GB).
...
**Live sources fail; that is normal.** Every fetcher raises `DataFetchError`
when it cannot supply a value for the point asked about — no coverage, a
service problem, or a cached file it cannot read — and `ConfigurationError` for
a missing cache or a bad argument, both subclasses of `UACPYError`. Catch that
one type and fall back. The split is by *who can act*: a `DataFetchError` says
try another source, a `ConfigurationError` says fix the call or the install.
