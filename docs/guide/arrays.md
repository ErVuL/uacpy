...
below are condensed from it, so the script is the authoritative figure
code. The figures are set
...
### That gain against isotropic noise **is** the sonar equation's `DI`

The number in the previous paragraph is the one the
[sonar equation](sonar.md#the-terms) calls the **directivity index**. No
function in `uacpy` computes it, because for the standard case there is nothing
to compute: a uniform line array at `d = λ/2` has uncorrelated noise at every
element pair, so its gain against an isotropic field is exactly
...
stops applying at all and you owe the budget a measured or modelled **array
gain** `AG` instead — the term that replaces `DI` in
[`sonar.md`](sonar.md#the-terms).

**`DI` is a budget term, not a correction you apply to data.** It belongs in
the sonar equation, where `NL` is a per-element noise level and `DI` accounts
for the beamformer you have not run yet. It does **not** belong on the output
of [`beamform`](#beamform--the-model-facing-entry-point), which has already
collected the gain inside `|eᴴ·p|` — see §3. Counting it in both places is a
`10·log10(N)` double-count — 12 dB of free figure of merit for a 16-element
array, and a detection range to match.
...
spectra move, by 1e-16 to 1e-13 relative. Whether a given call moves is a
property of the problem size, not of the function: OpenBLAS leaves small GEMMs
single-threaded, so a 16-element array over 200 snapshots barely shifts while a
64-element array over 4000 snapshots shifts almost everywhere.

Nothing *discrete* moved in any of that. Peak bearings were identical at both
problem sizes, and the JANUS detector in `uacpy.comms` — the same linear
algebra feeding a threshold test — returned an identical detection index in
750 of 750 real detections at one thread versus eight, even though its
underlying test statistic did move in 95 of them, by at most 4.0e-16 relative.
Perturbing that statistic on purpose by ±1, ±8, ±64 and ±4096 ULP flipped no
index either, across 749 realisations: the smallest GO-CFAR tie margin measured
there was 3.05e-04, about twelve orders of magnitude clear of float64 epsilon.

So treat this as a bit-identity problem, not a correctness one. Bearings,
detections and anything else you *decide* are safe to carry between machines;
bytes are not. If you need identical bits — a hash, a stored regression
baseline, a figure you will diff — pin both variables to the same value on
every machine before NumPy is imported, because moving the same script from a
laptop to a cluster node, which sets `OMP_NUM_THREADS` for you, silently
changes the thread count and with it the last digits.
