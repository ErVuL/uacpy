...
| `DI` | receiving directivity index | `+` | [array processing](arrays.md) |
...
[`docs/figure_scripts/sonar.py`](../figure_scripts/sonar.py) — the code below
...
| `ts_ellipsoid(a_m, b_m, c_m, *, frequency_hz=None, sound_speed=1500.0)` | `20·log10(bc/2a)` | viewed along `a` | `ka > 10` |
...
| `ts_plate(width_m, height_m, frequency_hz, *, angle_deg=0.0, sound_speed=1500.0)` | `10·log10[(wh/λ)²·sinc²β·cos²θ]` | **from normal incidence** | `ka > 1` on the smaller dimension |
...
| `mvdr(K, replicas, loading=1e-2)` | `P_MV = 1 / (eᴴK⁻¹e)`, max scaled to 1 |
...
sonar.mvdr(k_matched, bank, loading=1e-3)
